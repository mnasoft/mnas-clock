  ...
  (with-slots (rotation) window
    (let* ((w (window-width window))
      ...
      (multiple-value-bind (ss mm hh) (decode-universal-time (get-universal-time))
        ...
        (gl:vertex (* ss-l (sin (* *2pi* (/ ss 60))))
                   (* ss-l (cos (* *2pi* (/ ss 60)))))
        (gl:color 0.0 1.0 0.0)
        (gl:vertex (* mm-l (sin (* *2pi* (/ (+ mm (/ ss 60)) 60))))
                   (* mm-l (cos (* *2pi* (/ (+ mm (/ ss 60)) 60)))))
        (gl:color 1.0 0.0 1.0)
        (gl:vertex (* hh-l (sin (* *2pi* (/ (mod (+ hh (/ mm 60) (/ ss 3600)) 12) 12))))
                   (* hh-l (cos (* *2pi* (/ (mod (+ hh (/ mm 60) (/ ss 3600)) 12) 12))))))
      (gl:end)
      (gl:begin :triangles)
      ...
      (multiple-value-bind (ss mm hh) (decode-universal-time (get-universal-time))
        (let ((ss-a (- *pi/2* (* ss 2/60 3.1415927)))
          ...
          (apply 'gl:vertex (vlisp:polar '(0 0) hh-a hh-l))
          (apply 'gl:vertex (vlisp:polar '(0 0) (- hh-a *pi/2*) 0.06))
          (gl:color 0.0 1.0 0.0)
          (apply 'gl:vertex (vlisp:polar '(0 0) (+ mm-a *pi/2*) 0.05))
          (apply 'gl:vertex (vlisp:polar '(0 0) mm-a mm-l))
          (apply 'gl:vertex (vlisp:polar '(0 0) (- mm-a *pi/2*) 0.05))
          (gl:color 1.0 0.0 0.0)
          (apply 'gl:vertex (vlisp:polar '(0 0) (+ ss-a *pi/2*) 0.04))
          (apply 'gl:vertex (vlisp:polar '(0 0) ss-a ss-l))
          (apply 'gl:vertex (vlisp:polar '(0 0) (- ss-a *pi/2*) 0.04))
          (gl:end))))))

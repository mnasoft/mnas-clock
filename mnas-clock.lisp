...
(defmethod render ((window simple-window))
  ...
  (with-slots (rotation) window
    ...
    (loop :for i :from 0 :to 100 :do
      (gl:vertex (* 1.5 (sin (* 2 3.1415927 (/ i 100))))
                 (* 1.5 (cos (* 2 3.1415927 (/ i 100))))))
    (gl:end)
    (gl:point-size 10)
    ...
    (loop :for i :from 0 :to 12 :do
      (gl:vertex (* 1.4 (sin (* 2 3.1415927 (/ i 12))))
                 (* 1.4 (cos (* 2 3.1415927 (/ i 12))))))
    (gl:vertex 0.0  0.0 0.0)
    (multiple-value-bind (ss mm hh) (decode-universal-time (get-universal-time))
      (gl:color 0.0 0.0 0.0)
      (gl:vertex (* 1.4 (sin (* 2 3.1415927 (/ ss 60))))
                 (* 1.4 (cos (* 2 3.1415927 (/ ss 60)))))
      (gl:color 0.0 1.0 0.0)
      (gl:vertex (* 1.3 (sin (* 2 3.1415927 (/ (+ mm (/ ss 60)) 60))))
                 (* 1.3 (cos (* 2 3.1415927 (/ (+ mm (/ ss 60)) 60)))))
      (gl:color 1.0 0.0 1.0)
      (gl:vertex (* 1.2 (sin (* 2 3.1415927 (/ (mod (+ hh (/ mm 60) (/ ss 3600)) 12) 12))))
                 (* 1.2 (cos (* 2 3.1415927 (/ (mod (+ hh (/ mm 60) (/ ss 3600)) 12) 12))))))
    (gl:end)
    (gl:begin :triangles)
    (gl:color 1.0 0.0 0.0)
    (multiple-value-bind (ss mm hh) (decode-universal-time (get-universal-time))
      (let ((ss-a (- (/ 3.1415927 2) (* ss 2/60 3.1415927)))
            (mm-a (- (/ 3.1415927 2) (* (+ mm (/ ss 60)) 2/60 3.1415927)))
            (hh-a (- (/ 3.1415927 2) (* (mod (+ hh (/ mm 60) (/ ss 3600)) 12) 2/12 3.1415927))))
        (apply 'gl:vertex (vlisp:polar '(0 0) (+ ss-a (/ 3.1415927 2)) 0.04))
        (apply 'gl:vertex (vlisp:polar '(0 0) ss-a 1.4))
        (apply 'gl:vertex (vlisp:polar '(0 0) (- ss-a (/ 3.1415927 2)) 0.04))
        (gl:color 0.0 1.0 0.0)
        (apply 'gl:vertex (vlisp:polar '(0 0) (+ mm-a (/ 3.1415927 2)) 0.04))
        (apply 'gl:vertex (vlisp:polar '(0 0) mm-a 1.2))
        (apply 'gl:vertex (vlisp:polar '(0 0) (- mm-a (/ 3.1415927 2)) 0.04))
        (gl:color 1.0 0.0 1.0)
        (apply 'gl:vertex (vlisp:polar '(0 0) (+ hh-a (/ 3.1415927 2)) 0.04))
